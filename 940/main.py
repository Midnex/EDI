...
# When its time to rewrite this, can split all ~ with \n. Followed by reading in each lines
# from here we can check each line for the segment, such as ST, N1, W04, etc. Allowing us
# to create a file that doesn't break when it runs into issues. Below is what i want to call
# a brute force parser. Any 1 item changes, it can break it. Such as having multiple SKUs Per
# Transaction Set Header (STLoop)
...
def parse(pInput, pOutput):
    ...
    index = 0
    tempData = []
    for i in range(3, len(data) - 1): # add another loop in to pick up trans with multiple sku.
        if set == 11:
            set = 0
